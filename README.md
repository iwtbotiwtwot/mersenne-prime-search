...
SAM treats a Mersenne candidate as an interaction between two informational
objects orbiting within a rational closed 2D sphere.

The first object is the reciprocal Mersenne orbit generated by

$$
\alpha=2+\sqrt{3}.
$$

Its common component reproduces the Lucas--Lehmer recurrence while its second
component retains the reciprocal direction discarded by the ordinary scalar
readout. The second object is the prime-ruler orbit

$$
\beta_p=\frac{p+\sqrt{3}}{p-\sqrt{3}},
$$

which places the prime exponent $p$ on the same norm-one informational
surface. Stereographic coordinate $x=\tan(\chi/2)$ closes the plane into the
sphere, where reciprocal motion $x\leftrightarrow1/x$ is reversible. The two
objects carry nested dyadic beats; their self, prime-mark, and transition
echoes identify candidate contact coordinates.

Each contact is translated to the exact Mersenne factor coordinate
...
For a prime factor $q$, the multiplicative order of 2 modulo $q$ is $p$.
Exact modular arithmetic certifies every factor. A candidate with no factor
remains unresolved until an exact Lucas--Lehmer test assigns primality.
